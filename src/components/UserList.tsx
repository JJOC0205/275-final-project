...
export function UserList(): JSX.Element {
    ...
    function updateUserMovies(newMovie: MovieProps) {
        const movieFilter = testMovies.filter((movie) => newMovie.title === movie.title);
    ...
    return (
        <div ref={drop} style={{ backgroundColor: isOver ? 'red' : 'white', width: "510px", height:"1000px", border: "1px solid#000"}}>
            {userMovies.map((movie) => {
                return <div className="ListItem" key={movie.id}>
                    <Movie id={movie.id} title={movie.title} released={movie.released} runtime={movie.runtime} watched={movie.watched} description={movie.description} rating={movie.rating}></Movie>
                </div>
            })}
        </div>
    )
